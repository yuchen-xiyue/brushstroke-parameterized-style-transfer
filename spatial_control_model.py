...
def get_guided_gram_matrices(features, maps):
    """
...
def guided_style_loss(features_lhs, features_rhs, layers, weights, content_maps, style_maps):
    """
    ...
    maps_lhs = [content_maps[key] for key in layers]
    maps_rhs = [style_maps[key] for key in layers]
    gram_matrices_lhs = get_guided_gram_matrices(feat_lhs, maps_lhs)
    gram_matrices_rhs = get_guided_gram_matrices(feat_rhs, maps_rhs)
...
class BrushstrokeOptimizer:
    ...
    def __init__(self,
        ...
        self.content_maps = {
            'conv1_1': [cm.resize((self.canvas_width//2, self.canvas_height//2), refcheck=False) for cm in content_maps1_1], 
            'conv2_1': [cm.resize((self.canvas_width//4, self.canvas_height//4), refcheck=False) for cm in content_maps2_1], 
        ...
        self.style_maps = {
            'conv1_1': [sm.resize((self.canvas_width//2, self.canvas_height//2), refcheck=False) for sm in style_maps2_1], 
            'conv2_1': [sm.resize((self.canvas_width//4, self.canvas_height//4), refcheck=False) for sm in style_maps2_1], 
            'conv3_1': [sm.resize((self.canvas_width//8, self.canvas_height//8), refcheck=False) for sm in style_maps3_1], 
            'conv4_1': [sm.resize((self.canvas_width//16, self.canvas_height//16), refcheck=False) for sm in style_maps4_1], 
            'conv5_1': [sm.resize((self.canvas_width//32, self.canvas_height//32), refcheck=False) for sm in style_maps5_1]}
