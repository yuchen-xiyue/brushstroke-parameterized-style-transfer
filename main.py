...
parser.add_argument('-g', '--guided')
args = parser.parse_args()

if args.guided:
	for cont_img, styl_img in zip(cont_imgs, styl_imgs):
		cont_img = Image.open(path.join(CONT_PATH, cont_img))
		cont_name = cont_img.split('.')[0]
		...
		styl_img = Image.open(path.join(STYL_PATH, styl_img))
		styl_name = styl_img.split('.')[0]
